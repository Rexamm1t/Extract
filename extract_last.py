...
VERSION = "EXTRACT 9.5.0"
VERSION_ALL = "EXTRACT 9.5.0 (8.7.25) (UNIX base)"
...
class Casino:
    ...
    def trade(self, command):
        if not self.current_user:
            print(f"{Fore.RED}Сначала выберите пользователя! (create/switch username")
            return
    ...
    try:
        while True:
            ...
            if casino.current_user:
                ...
                prompt = f"{Fore.BLUE}╭─{VERSION_ALL} - Для вывода списка команд введите - help\n╰─➤ {Style.RESET_ALL}"
