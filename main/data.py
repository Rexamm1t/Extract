...
class HistoryTracker:
    ...
    def get_timestamps(self, limit: int = 30) -> List[str]:
        timestamps = []
        for record in self.history[-limit:]:
            ts = record["timestamp"]
            try:
...
class CryptoMarket:
    ...
    def print_chart(self, coin: str, height: int = 12, width: int = 50):
        history = self.history_tracker.get_coin_history(coin)
        if len(history) < 2:
...
class Casino:
    ...
    def show_rates(self):
        try:
            ...
            old_rates = {coin: float(rate) for coin, rate in self.market.rates.items()}
        ...
        for coin, rate in self.market.rates.items():
            if coin == "EXTRACT":
                continue
            old_rate = old_rates.get(coin, float(rate))
            change = ((float(rate) - old_rate) / old_rate) * 100 if old_rate != 0 else 0
